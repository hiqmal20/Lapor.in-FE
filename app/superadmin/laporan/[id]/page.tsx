...
export default function SuperAdminLaporanDetailPage() {
    ...
    if (ok) {
      setLaporan((prev) =>
        prev
          ? {
              ...prev,
              status: selectedStatus as LaporanDetail["status"],
  ...
  const handleSendComment = async () => {
    ...
    if (ok) {
      ...
      setComments((prev) => [
        ...
        {
          ...data.comment,
          user_name: data.comment.user_name ?? user.name ?? "Super Admin",
  ...
  const handleDeleteComment = async (cId: number) => {
    const { ok } = await apiFetch(`/api/laporin/laporan/comments/${cId}`, {
      method: "DELETE",
    });
  ...
  const handleDeleteLaporan = async () => {
    ...
    try {
      ...
      if (ok) {
        alert("Laporan berhasil dihapus oleh Super Admin!");
    ...
    >
      <div className="grid grid-cols-3 gap-5 items-start">
        <div className="col-span-2 space-y-4">
          {/* Top Actions: Back Button & Admin Delete Button */}
          ...
          <div className="bg-white border border-gray-100 rounded-xl p-5">
            ...
            <div className="flex gap-2 items-center">
              ...
              <input
                ...
                className="flex-1 h-9 px-3 text-xs border border-gray-200 rounded-lg bg-white focus:outline-none focus:border-violet-400 transition-colors"
              />
              <button
