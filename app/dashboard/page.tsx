    ...
    <div className={`space-y-6 transition-all duration-500 ${visible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"}`}>
      ...
      >
        <div className="absolute right-0 top-0 w-40 h-full opacity-20 pointer-events-none">
          ...
          <div className="w-24 h-24 bg-white rounded-full absolute -right-5 bottom-0 animate-pulse" style={{ animationDelay: "300ms" }} />
        </div>
        ...
        <Button asChild size="sm" className="bg-white text-blue-600 hover:bg-blue-50 hover:scale-105 transition-all duration-200">
        ...
        {stats.map((stat, i) => (
          ...
          >
            <CardContent className="p-4 flex items-center justify-between">
              ...
              <div className={`p-2 rounded-lg ${stat.bg} transition-transform duration-200 hover:scale-110`}>
        ...
        <CardContent className="p-4">
              ...
              <>
                ...
                {approved > 0 && (
                  <div
                    ...
                    style={{ width: `${(approved / total) * 100}%`, transitionDelay: "100ms" }}
                  />
                )}
                {rejected > 0 && (
                  <div
                    className="bg-red-500 h-full transition-all duration-700 ease-out"
                    style={{ width: `${(rejected / total) * 100}%`, transitionDelay: "200ms" }}
                  />
                )}
              </>
            ) : (
          ...
          <div className="flex gap-4 mt-3">
            {[
              ...
              <span key={s.label} className="text-xs text-gray-500 flex items-center gap-1">
                <span className={`w-2 h-2 ${s.color} rounded-full inline-block`} />
                {s.label} ({s.count})
              </span>
            ))}
          </div>
      ...
      <Card className="border border-gray-100 shadow-none">
        <CardContent className="p-4">
          <div className="flex items-center justify-between mb-4">
            ...
            <Link href="/dashboard/laporan" className="text-xs text-blue-600 hover:underline flex items-center gap-1 hover:gap-2 transition-all duration-150">
          ...
          {loading ? (
            <div className="space-y-3">
              {[1, 2, 3].map((i) => (
                <div key={i} className="h-12 bg-gray-50 rounded-lg animate-pulse" style={{ animationDelay: `${i * 100}ms` }} />
              ))}
            </div>
        ...
        {[
          {
            ...
            icon: Plus, bg: "bg-blue-50", iconColor: "text-blue-600",
            title: "Submit New Report", desc: "File a public complaint or issue",
          },
          {
            href: "/dashboard/laporan",
            icon: FileText, bg: "bg-green-50", iconColor: "text-green-600",
            title: "View My Reports", desc: "Track all your submitted reports",
          },
        ].map((action, i) => (
            ...
            <Card className="border border-gray-100 shadow-none hover:shadow-md hover:-translate-y-0.5 transition-all duration-200 cursor-pointer group">
              <CardContent className="p-4 flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <div className={`p-2 ${action.bg} rounded-lg group-hover:scale-110 transition-transform duration-200`}>
                ...
                <ArrowRight size={16} className="text-gray-400 group-hover:translate-x-1 transition-transform duration-200" />
              </CardContent>
            </Card>
          </Link>
        ))}
