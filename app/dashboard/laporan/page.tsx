...
export default function MyLaporanPage() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      if (lRes.ok)
        setLaporan(
          Array.isArray(lRes.data) ? lRes.data : (lRes.data.data ?? []),
        );
      if (cRes.ok) {
  ...
  const getCategoryName = (row: any) => {
    ...
    if (row.category && typeof row.category === "object") {
      if (row.category.name) return String(row.category.name).trim();
      if (row.category_name) return String(row.category_name).trim();
    }
    return "";
  };
  ...
  const filtered = laporan.filter((l) => {
    ...
    const matchCat =
      ...
      catName.toLowerCase().includes(categoryFilter.trim().toLowerCase());
  ...
  return (
    <div
      className={`flex flex-col h-full min-h-[calc(100vh-3rem)] space-y-4 transition-all duration-400 ${visible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-3"}`}
    >
      {/* header */}
      ...
      <div className="flex gap-1 border-b border-gray-100">
        {STATUS_TABS.map((tab) => {
          const count =
            tab.key === "all" ? laporan.length : countByStatus(tab.key);
      ...
      <div
        className={`flex-1 transition-opacity duration-150 ${fading ? "opacity-0" : "opacity-100"}`}
      >
        {loading ? (
              ...
              return (
                <div
                  ...
                  style={{
                    ...
                    transition: `opacity 0.3s ease ${idx * 50}ms, transform 0.3s ease ${idx * 50}ms, box-shadow 0.2s, border-color 0.2s`,
                  }}
                >
                  <div className="flex gap-3">
                    {/* thumbnail */}
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-start justify-between gap-3">
                        <div className="flex items-center gap-2 flex-wrap">
                          <button
                            onClick={(e) => {
                              e.stopPropagation();
                              const cat =
                                getCategoryName(item) || "All Categories";
                              setCategoryFilter(cat);
                              setActiveTab("all");
                              setShowDropdown(false);
                            }}
                            className={`text-xs px-2 py-0.5 rounded-full font-medium ${getCatColor(getCategoryName(item))} cursor-pointer`}
                          >
                            {getCategoryName(item) || "General"}
                          </button>
                          {item.priority && (
                            <span
                              className={`text-xs px-2 py-0.5 rounded-full font-medium capitalize ${PRIORITY_COLOR[item.priority] ?? PRIORITY_COLOR.low}`}
                            >
                              {item.priority}
                            </span>
                          )}
                        ...
                        <div
                          className={`flex items-center gap-1 px-2 py-1 rounded-full text-xs font-medium capitalize shrink-0 ${statusInfo.color}`}
                        >
                          {statusInfo.icon}
