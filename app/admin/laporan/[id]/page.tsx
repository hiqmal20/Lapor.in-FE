...
import {
  ArrowLeft, MapPin, Calendar, Clock, CheckCircle,
  XCircle, Tag, MessageSquare, User, Send, Trash2,
  AlertTriangle,
} from "lucide-react";
...
export default function AdminLaporanDetailPage() {
  ...
  const handleUpdateStatus = async () => {
    setUpdateError("");
    setUpdating(true);
    const { ok, data } = await apiFetch(`/api/laporin/laporan/${id}/status`, {
      method: "PUT",
      body: JSON.stringify({ status: selectedStatus, admin_note: adminNote }),
    });
    setUpdating(false);
    if (ok) {
      setLaporan((prev) =>
        prev ? { ...prev, status: selectedStatus as LaporanDetail["status"], admin_note: adminNote } : prev
      );
  ...
  const handleSendComment = async () => {
    ...
    if (ok) {
      ...
      setComments((prev) => [
        ...prev,
        { ...data.comment, user_name: data.comment.user_name ?? user.name ?? "Admin", created_at: new Date().toISOString() },
      ]);
      setCommentText("");
    }
  };
  ...
  const handleDeleteComment = async (commentId: number) => {
    const { ok } = await apiFetch(`/api/laporin/laporan/comments/${commentId}`, { method: "DELETE" });
  ...
  if (!laporan) {
    return (
      <div className="text-center py-20 text-gray-400">
        ...
        <Button variant="ghost" size="sm" className="mt-3 text-emerald-600" onClick={() => router.back()}>
          Go back
        </Button>
      </div>
    );
  ...
  const imgUrl = rawImg
    ? rawImg.startsWith("http") ? rawImg : `${process.env.NEXT_PUBLIC_API_URL}/uploads/${rawImg}`
    : null;
    ...
    <div className={`transition-all duration-300 ${visible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-3"}`}>
      <div className="grid grid-cols-3 gap-5 items-start">
        ...
        <div className="col-span-2 space-y-4">
          ...
          {imgUrl && (
            <div className="bg-white border border-gray-100 rounded-xl p-5">
              <h2 className="text-sm font-semibold text-gray-900 mb-3">Attached Photo</h2>
              <img src={imgUrl} alt="Report" className="rounded-lg w-full max-h-72 object-cover" />
            </div>
          )}
        ...
        <div className="col-span-1 space-y-4">
            ...
            <div className="space-y-3">
              ...
              <div>
                ...
                <textarea
                  ...
                  className="w-full px-3 py-2 text-xs border border-gray-200 rounded-lg bg-white focus:outline-none focus:border-emerald-400 transition-colors resize-none"
                />
              </div>
              ...
              <Button
                ...
                className="w-full h-9 text-xs bg-emerald-600 hover:bg-emerald-700 text-white transition-all duration-200 disabled:opacity-50"
              >
                {updating
                  ? <span className="flex items-center gap-2"><span className="w-3 h-3 border-2 border-white/30 border-t-white rounded-full animate-spin" />Updating...</span>
                  : "Update Status"}
              </Button>
            </div>
          </div>
          ...
          <div className="bg-white border border-gray-100 rounded-xl p-5">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-sm font-semibold text-gray-800">{laporan.user_name}</p>
