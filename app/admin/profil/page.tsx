...
import {
  User, Mail, Phone, MapPin, Pencil, X, Save, Lock, CheckCircle, Calendar,
} from "lucide-react";
...
export default function AdminProfilPage() {
  ...
  useEffect(() => {
    ...
    apiFetch("/api/laporin/auth/me").then(({ ok, data }) => {
      if (ok) {
        ...
        setProfile(d);
        setEditForm({ name: d.name, phone: d.phone ?? "", address: d.address ?? "" });
        const token = localStorage.getItem("token") ?? "";
        setAuth({ ...d, role: d.role as "user" | "admin" | "super_admin" }, token);
      }
    });
  ...
  return (
    <div className={`space-y-5 max-w-2xl transition-all duration-300 ${visible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-3"}`}>
      ...
      <div className="bg-white border border-gray-100 rounded-xl p-5">
        <div className="flex items-start justify-between gap-4">
          <div className="flex items-center gap-4">
            <div className="w-16 h-16 rounded-xl bg-indigo-600 flex items-center justify-center text-white text-2xl font-bold shrink-0">
            ...
            <div>
              ...
              <div className="flex items-center gap-2 mt-2">
                <span className="text-xs px-2 py-0.5 rounded-full bg-indigo-50 text-indigo-700 font-medium flex items-center gap-1">
                  <User size={10} />
          ...
          {!editing ? (
            <Button size="sm" variant="outline" onClick={() => setEditing(true)}
              className="text-xs gap-1.5 border-gray-200 hover:border-indigo-400 hover:text-indigo-600 transition-all duration-200 shrink-0">
      ...
      <div className={`bg-white border rounded-xl p-5 transition-all duration-200 ${editing ? "border-indigo-200 shadow-sm" : "border-gray-100"}`}>
          ...
          <div>
            ...
            {editing ? (
              <Input name="name" value={editForm.name} onChange={handleChange}
                className="h-9 text-sm border-gray-200 focus:border-indigo-400 bg-gray-50 rounded-lg transition-colors" />
            ) : (
          ...
          <div>
            ...
            {editing ? (
              <Input name="address" value={editForm.address} onChange={handleChange}
                placeholder="Jl. Gatot Subroto No. 45"
                className="h-9 text-sm border-gray-200 focus:border-indigo-400 bg-gray-50 rounded-lg transition-colors" />
            ) : (
              <p className="text-sm text-gray-800 flex items-center gap-2">
      ...
      <div className="bg-white border border-gray-100 rounded-xl p-5">
        <div className="flex items-center justify-between">
          <h3 className="text-sm font-semibold text-gray-900 flex items-center gap-2">
            <Lock size={14} className="text-indigo-500" />Security
          </h3>
          <button
            ...
            {[
              ...
              <div key={key}>
                ...
                <Input
                  ...
                  className="h-9 text-sm border-gray-200 focus:border-indigo-400 bg-gray-50 transition-colors"
                />
              </div>
            ))}
            <Button onClick={handleChangePassword} disabled={savingPassword}
              className="w-full h-9 text-sm bg-indigo-600 hover:bg-indigo-700 text-white transition-all duration-200">
              {savingPassword
