  ...
  return (
    <div className="min-h-screen grid lg:grid-cols-2">
      ...
      <div className={`flex flex-col items-center justify-center p-8 bg-gray-50 transition-all duration-700 ${visible ? "opacity-100 translate-x-0" : "opacity-0 -translate-x-6"}`}>
        <div className="w-full max-w-sm">
          ...
          <div className="mb-8">
            ...
            <p className="text-sm text-gray-500 mt-1">Isi data di bawah untuk mendaftar</p>
          </div>
      ...
      <div className={`hidden lg:flex flex-col justify-between bg-[#0f1f3d] p-12 relative overflow-hidden transition-all duration-700 ${visible ? "opacity-100 translate-x-0" : "opacity-0 translate-x-6"}`}>
        <div className="absolute top-[-80px] right-[-80px] w-72 h-72 rounded-full bg-blue-800/30 blur-[100px]" />
        <div className="absolute bottom-[-60px] left-[-60px] w-80 h-80 rounded-full bg-indigo-900/40 blur-[120px]" />
        ...
        <div className="relative z-10 space-y-8">
          <div>
            ...
            <p className="text-blue-200/70 mt-4 text-base leading-relaxed max-w-sm">
              Jadilah bagian dari komunitas yang peduli. Setiap laporan yang kamu kirim membantu menciptakan lingkungan yang lebih baik.
            </p>
          </div>
          <div className="space-y-4">
            {[
              { icon: FileText, title: "Laporan real-time", desc: "Pantau status laporan kapan saja" },
              { icon: Shield, title: "Privasi terjaga", desc: "Identitasmu aman bersama kami" },
              { icon: Users, title: "Komunitas aktif", desc: "Ribuan pengguna sudah bergabung" },
            ].map((f, i) => (
